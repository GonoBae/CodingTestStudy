...
int BFS(const std::vector<std::vector<int>>& array, std::vector<bool>& chk, int start);
...
int main() {
	...
	std::vector<std::vector<int>> array(point);

	for (int i = 0; i < cnt; i++) {
		int a, b;
		std::cin >> a >> b;
		array[b - 1].push_back(a - 1);
	}
